...
export function App() {
  ...
  async function iniciar() {
    ...
    watcherRef.current = navigator.geolocation.watchPosition(
      ...
      (error) => setMensagem(error.code === error.PERMISSION_DENIED
        ? 'A permissão foi negada. Libere a localização nas configurações do navegador e tente novamente.'
        : 'Ainda não conseguimos obter a posição. Mantenha o GPS ligado.'),
  ...
  async function parar() {
    setEstado('parando');
    limparObservador();
    if (finderTokenRef.current) {
      await supabase.rpc('end_tracking_session', { p_finder_token: finderTokenRef.current });
    }
    setEstado('encerrado');
  ...
  return (
    <main className="page">
      <section className="hero">
        <span className="eyebrow">TO AQUI</span>
        <h1>Você encontrou este QR Code?</h1>
        <p>Ajude a família enviando sua localização com segurança.</p>
      </section>
      <section className="card">
        <div className={`location-icon ${estado === 'encerrado' ? 'done' : ''}`} aria-hidden="true">
          {estado === 'encerrado' ? '✓' : '⌖'}
        </div>
        <div>
          <h2>{ativo ? 'Localização sendo enviada' : estado === 'encerrado' ? 'Compartilhamento encerrado' : 'Compartilhar localização'}</h2>
          <p className="message">{mensagem || 'Ao continuar, o navegador pedirá sua permissão. Somente os responsáveis desta família verão a posição enviada.'}</p>
        </div>
        ...
        {ativo && <button className="secondary danger" onClick={parar} disabled={estado === 'parando'}>{estado === 'parando' ? 'Encerrando…' : 'Parar de compartilhar'}</button>}
      </section>
